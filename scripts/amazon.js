...
// import { refreshcart } from './checkout.js';
...
document.querySelectorAll('.js-add-to-cart').forEach((button)=>{
  button.addEventListener('click',()=>{
    ...
    addItem(productName);
  updateCartQuatity(cart);
    // refreshcart(cart);
  })
})

function updateCartQuatity(cart){
  let cartQuatity=0;
  cart.forEach((item)=>{
    cartQuatity+=Number(item.quatity);
  })
  document.querySelector('.cart-quantity').innerHTML=cartQuatity;
}
